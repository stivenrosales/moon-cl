    ...
    <div className="space-y-8 md:space-y-10">
      <header className="flex flex-wrap items-end justify-between gap-4">
        ...
        {isAdmin ? (
          <Button asChild variant="gold">
            <Link href="/admin#rondas">Crear ronda</Link>
          </Button>
        ) : null}
      </header>
      ...
      {open.length > 0 ? (
        ...
        <Card className="p-10 text-center">
          <Vote className="mx-auto h-8 w-8 text-primary/60" />
          <p className="mt-4 hand-script text-2xl text-foreground">No hay ronda activa</p>
  ...
  return (
    <Link href={`/rondas/${round.id}`} className="block focus-ring rounded-2xl">
      <Card className="p-6 transition-all hover:-translate-y-0.5 hover:shadow-[0_30px_60px_-30px_rgba(0,0,0,0.5)]">
        ...
        <h3 className="display mt-3 text-2xl leading-tight">{round.title}</h3>
        {round.description ? (
          <p className="mt-2 text-sm text-muted-foreground line-clamp-2">{round.description}</p>
        ) : null}
